...
Companion to: `docs/lp-dashboard-prd.md`, `docs/safehaven-integration.md`
...
- **BaaS adapter** — provider-agnostic `services/baas` (interface, neutral types, normalised `TransferStatus`, registry). Safe Haven now an adapter (`services/baas/safehaven/adapter.go`) selected by `BAAS_PROVIDER` in `main.go`. All consumers (admin funding, funding actions, auto-pay, reconcile, balance, webhook dispatch) depend only on `baas.Default()`. **Swapping providers = add one adapter + one `main.go` case.**
...
- **C3 webhook** — route-B/C branch converges `fiat_payout_status` (`controllers/safehaven_webhook.go`).
...
**Pre-existing, not introduced here:** two `safehaven_webhook_test.go` cases fail locally because `.env` sets `ENVIRONMENT=production` and those tests don't override it (they assert the dev path). Verified failing on a clean tree too.

> **Architecture rule for this work:** do not invent new layers. New read endpoints go on the existing `providerRoutes` group; new money-movement logic hooks into the existing Sui settle path and the existing Safe Haven client; new periodic work is a gocron job in `tasks.StartCronJobs`; new persisted state is an ent field/entity regenerated with `make gen-ent`. Match `u.APIResponse`, the `"provider"` context key, and the `storage.Client` query style verbatim.
...
- **Safe Haven client is wired at boot** — `main.go:40-49` constructs it via `safehaven.NewClientFromCredentials(...)` and registers it with `safehaven.SetDefault`. Global access: `safehaven.Default()` (`services/baas/safehaven/setup.go`). Methods `Transfer`, `NameEnquiry`, `TransferStatus`, `GetAccount`, `ListAccounts`, `CreateSubAccount`, `InitiateIdentity`, `ValidateIdentity` all implemented (`services/baas/safehaven/client.go`).
- **Webhook route is registered** — `POST /v1/safehaven/webhook` (`routers/index.go:69`), with HMAC-SHA256 signature verify already written (`controllers/safehaven_webhook.go:90-95`). Only the route-B/C **branch body** is a TODO (`:81`).
- **Config exists** — `config.SafehavenConfig()` (`config/safehaven.go`) with `ClientID`, `BaseURL`, `WebhookSecret`, `MaxTransferNGN`, debit account, etc.
...
|---|---|---|---|---|---|
| C1 | Auto-pay: fire Safe Haven `Transfer` from the LP's sub-account after on-chain settle | wiring | G2b (blocker) | **P0** | `services/sui_event_indexer.go`, new `services/order/fiat_payout.go`, `ent/schema/lockpaymentorder.go` |
...
| C3 | Implement webhook route-B/C branch (mark payout success/failed, idempotent) | wiring | G3 | **P0** | `controllers/safehaven_webhook.go` |
...
```go
// Safe Haven fiat payout (Route B) — set when we debit the LP sub-account.
...
New file `services/order/fiat_payout.go`, following the stateless-constructor pattern (`services/api_key.go:16-22`) and the `safehaven.Default()` access pattern. Keep the money-movement logic out of the indexer — the indexer just calls this service (matches how `provider.go:429` calls `orderService.NewOrderSui().SettleOrder`).
...
// PayoutForSettledOrder debits the LP's delegated Safe Haven sub-account to pay
...
func (s *FiatPayoutService) PayoutForSettledOrder(ctx context.Context, lockOrderID uuid.UUID) {
    sh := safehaven.Default()
    if sh == nil {
        logger.Warnf("fiat payout %s: Safe Haven not configured; skipped", lockOrderID)
        return
    }
    ...
    if p == nil || p.SafehavenAccountNumber == "" {
    ...
    // name-enquiry → transfer (the Safe Haven primitive)
    ...
    tr, err := sh.Transfer(ctx, safehaven.TransferRequest{
        NameEnquiryReference: ne.SessionID,
        DebitAccountNumber:   p.SafehavenAccountNumber,
...
Per the explicit decision in `docs/safehaven-integration.md:94-106` (**settle-after-finality** for Route B — LP funds at stake), trigger the payout **after** `updateOrderStatusSettled` commits the `settled` status. In `services/sui_event_indexer.go`, after `tx.Commit()` (≈`:728`) and alongside the existing webhook/SSE fan-out:

```go
// Route B: debit the LP's Safe Haven sub-account to pay the recipient.
...
Rationale (first-principles / debuggability): the on-chain settle is the source of truth; fiat payout is a downstream effect with its own recorded status. Decoupling them (goroutine + status field) means a Safe Haven outage never corrupts settlement, and every payout's state is inspectable on the order row.

### 2.4 C3 — Webhook route-B/C branch

Replace the TODO at `controllers/safehaven_webhook.go:81`. Idempotent, derives the order from the reference:
...
case strings.HasPrefix(p.PaymentReference, "routeB-"), strings.HasPrefix(p.PaymentReference, "routeC-"):
    ...
    if err != nil { logger.Warnf("[safehaven] bad ref %s", p.PaymentReference); break }
    ...
    default:
        logger.Infof("[safehaven] payout pending ref=%s status=%s", p.PaymentReference, p.Status)
        break
    }
    _, err = storage.Client.LockPaymentOrder.Update().
        ...
        Save(ctx)
    if err != nil { logger.Errorf("[safehaven] payout update %s: %v", oid, err) }
```

(Handler already reads raw body + verifies signature + parses `safeHavenWebhookPayload` at `:40-87`; only this branch changes.)
...
func (ctrl *ProviderController) GetBalance(ctx *gin.Context) {
    ...
    // Naira float from Safe Haven (read-only GetAccount)
    if sh := safehaven.Default(); sh != nil && provider.SafehavenAccountID != "" {
        if acct, err := sh.GetAccount(ctx, provider.SafehavenAccountID); err == nil {
            ...
            logger.Warnf("GetBalance %s: safehaven: %v", provider.ID, err)
        }
    }
...
Note: don't block the response if Safe Haven is down — return what you have with the status, matching the indexer's tolerant style.
...
Guard behind `OnlyProviderMiddleware`. Follow the do-not's in `docs/safehaven-integration.md:62-90` — these calls have fees/side-effects, so gate carefully and keep the admin path as fallback. Until shipped, Onboarding stays admin-gated (PRD §5.8).
...
2. **C1 + C3** together (payout fires + webhook confirms) behind the existing `safehaven.Default() == nil` guard so it's a no-op until Safe Haven creds are set — safe to merge dark.
...
**Codegen/migration:** every schema change (C2) is `make gen-ent` then the repo's migration path (`atlas.hcl` / `seed-db`). Add any new env keys (none strictly required — `SafehavenConfig` already has what C1 needs) to `.env.example` per `config/safehaven.go`.
...
3. **Dormant sub-accounts** — `docs/safehaven-integration.md:§1` flags accounts as `Dormant`; confirm a dormant sub-account can be debited before first live transfer.
...
5. **Balance read frequency** — `GetBalance` calls Safe Haven live per request; consider a short cache or a periodic snapshot field if Safe Haven rate-limits.
